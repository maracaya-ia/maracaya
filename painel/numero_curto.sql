...
-- order_id_cw interno (longo). Cardapio Web: display_id (sequencial da loja,
-- confirmado unico em 100% dos pedidos de todos os canais). external_display_id
-- foi testado e descartado: no 99Food repete dezenas de vezes (nao e por
-- pedido) e no iFood tambem tem colisao - display_id e o campo confiavel.
-- Saipos nao tem um numero curto separado -> usa o order_id_cw mesmo.
...
WITH ult AS (
    SELECT DISTINCT ON (loja_id, payload->>'id') loja_id, payload->>'id' AS oid,
           payload->>'display_id' AS numero
    FROM webhook_eventos
    WHERE tipo_evento IN ('polling', 'backfill') AND payload ? 'id'
    ORDER BY loja_id, payload->>'id', id DESC
)
-- UPDATE incondicional (sobrescreve valores gravados por uma versao anterior
-- desta migracao, que usava external_display_id e gerava numero duplicado)
